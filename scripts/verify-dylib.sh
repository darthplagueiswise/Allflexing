...
require_text "indexed Objective-C search" "com.allflexing.flex-objc-search-index" "$string_dump"
require_text "Objective-C function title" "Objective-C Functions" "$string_dump"
require_text "off-main Objective-C ABI validation" "Discovery and ABI validation run away from the main thread." "$string_dump"
...
require_text "explicit staged apply" "Apply and close app" "$string_dump"
