...
typedef struct {
    ...
    // Raw bit pattern for the secondary-scope typed profiles. For double/float
    // this holds the IEEE-754 bits; the stub reinterprets it with memcpy so the
    // compiler materializes the value into d0/s0 via a real fmov.
...
static double FLEXCallDoubleNoArgs(NSUInteger index) {
    ...
    FLEXCHookRecordHit(slot, enabled);
    if (enabled) {
        // Returning a double makes the compiler emit the AAPCS64 FP return:
        // the value lands in d0 via fmov. No machine code is hand-written.
...
+ (void)setEnabled:(BOOL)enabled forEntry:(FLEXHookEntry *)entry {
    ...
    FLEXCHookSlot *slot = &gFLEXCHookSlots[entry.runtimeSlot];
    // Bool profiles keep using forceValue; the secondary-scope profiles read
    // forceRawValue (signed/unsigned int, pointer bits, or IEEE-754 bits). All
    // are published before enabling so the stub never reads a stale value.
    ...
    switch (entry.abi) {
        ...
        case FLEXHookABICPointerNoArguments:
            // A fabricated pointer is only safe when the user supplied it
            // explicitly; zero stays NULL.
